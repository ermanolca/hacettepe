...
{
    private readonly HacettepeDbContext _context;

    public DataTableService(HacettepeDbContext context)
    {
        _context = context;
    }
    ...
    public DatatableResponse<T> GetDatatableObject(DatatableRequest request, IQueryable<T> model)
    {
        ...
        model = Search(model, request.Search.Value, request.Columns);
    ...
    private static IQueryable<T> Sort(IReadOnlyList<Sort> orders, IReadOnlyList<Column>? columns, IQueryable<T> model)
    {
        var column = FormatColumnName(columns[orders[0].Column].Data);

        var orderedQueryable = model.OrderBy(column + " " + orders[0].Dir);
    ...
    private static IQueryable<T> Search(IQueryable<T> model, StringValues searchValue, IEnumerable<Column>? columns)
    {
        if (columns == null)
        {
            return model;
        ...
        var enumerable = columns as Column[] ?? columns.ToArray();
        var paramList = new List<string>();
        var filterQuery = enumerable.Where(a => !string.IsNullOrEmpty(a.Search?.Value))
            .Select((x, i) =>
            {   
                paramList.Add(x.Search?.Value);
                return $"{x.Name} like '%@{i}%'";
            })
            .Aggregate(new StringBuilder(), (current, next) => current.Append(current.Length == 0? "" : " AND ").Append(next));

        if (paramList.Any())
        {
            model = model.Where(filterQuery.ToString(), paramList.ToArray()).AsQueryable();
        }
    ...
    /*private static ExpressionStarter<T> SearchAColumn(ExpressionStarter<T> predicate, Column column, StringValues value)
    {
